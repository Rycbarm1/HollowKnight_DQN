from tool.WindowsAPI import key_check, GAME_START, GAME_START_1, GAME_STOP, GAME_OVER
import time
...
def count_self_reward(next_self_blood, self_hp):
    if next_self_blood - self_hp < 0:
        # print("hurt now hp : {}, next_hp {}".format(self_hp, next_self_blood))
...
def pause_game(paused):
    stop_flag = False if paused else True
    op, d = key_check()
    if GAME_STOP in op or paused:
        paused = True
        Debug('pause game', debug_level=DEBUG_LEVEL.NORMAL)
        time.sleep(1)

    if paused:
        Debug('paused', debug_level=DEBUG_LEVEL.NORMAL)
        while True:
            op, d = key_check()
            # pauses game and can get annoying.
            if GAME_START in op or (stop_flag and GAME_START_1 in op):
                paused = False
                Debug('start game', debug_level=DEBUG_LEVEL.NORMAL)
                break
            if GAME_OVER in op:
                Debug('game over', debug_level=DEBUG_LEVEL.NORMAL)
                exit()
            time.sleep(0.1)
    return False